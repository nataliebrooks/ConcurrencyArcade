...
func main() {    
    ...
    // get info from host about what machine they want to run on
    // and which port.
	var hostname string = ""
	...
	// give info to arcade to get registered.
	out, in, f := cs221.MakeConnection("localhost", 6666, "ers")
	out <- "ADD ers "+hostname+" "+port+" \n"
	out <- "\n"
	okay := (<-in)
	fmt.Println(okay)

	// start running host at the port given to the arcade.
	var p int
	p, _ = strconv.Atoi(port)
	cs221.HandleConnections(hostname, p, handleGame, "ers", nil)
